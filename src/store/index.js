...
export default new Vuex.Store({
    state: {
        ...
        editWindow: {show: false, type: '', data: {}},
    ...
    mutations: {
        ...
        setEditWindow: (store, info) => {
            if(store.editWindow.show && info.show) return;
            store.editWindow = {'show': info.show, 'type': info.type, 'data': info.data}
        },
        setCV(store, cv) {store.content = cv},
        ...
        setCVmenu: (store, show) => {store.showCVmenu = show},
    ...
    actions: {  
        download(context){
            ...
            var popupWin = window.open('', '_blank', 'width= 700px,height=900,location=no,left=200px, top=20px');
        ...
        saveAction: async (context, name) => {
            ...
            var res = await Axios.post(context.state.uri + "/saveCV", {"key": context.state.key, "content": context.state.content});
        ...
        deleteCV: async(context) => {
            var res = await Axios.post(context.state.uri + "/delete", {"key": context.state.key, "cvName": context.state.content.cvName});
            context.state.cvList = res.data;
        },
